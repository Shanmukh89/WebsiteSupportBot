...
def scrape_with_firecrawl(url):
    """Fallback to Firecrawl API for complex or anti-bot protected sites."""
    if not FIRECRAWL_API_KEY:
        print("  Firecrawl API key not set, skipping Firecrawl fallback", file=sys.stderr)
        return None
        
    print(f"  Using Firecrawl API fallback for {url}...", file=sys.stderr)
    try:
        headers = {
...
def main():
    ...
    # Fallback to Firecrawl API if local scraping methods failed to extract pages
    if not scraped_pages:
        print("  Local scraping produced 0 pages. Triggering Firecrawl fallback...", file=sys.stderr)
        fc_res = scrape_with_firecrawl(start_url)
        if fc_res:
            scraped_pages.append(fc_res)
